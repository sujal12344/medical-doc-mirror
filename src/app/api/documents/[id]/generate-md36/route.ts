...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const templatePath = path.join(process.cwd(), "format", "md-36", "03_Official_Form_MD-36_Sample_Taking_Intimation_Template.docx");
    ...
    return new NextResponse(buf as unknown as BodyInit, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="MD-36_Sample_Taking_Intimation.docx"`,
      },
    });
  } catch (error) {
    ...
    console.error("[generate-md36] Error:", error);
