...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const templatePath = path.join(process.cwd(), "format", "md-35", "02_Official_Form_MD-35_Seizure_Receipt_Template.docx");
    ...
    return new NextResponse(buf as unknown as BodyInit, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="MD-35_Seizure_Receipt.docx"`,
      },
    });
  } catch (error) {
    ...
    console.error("[generate-md35] Error:", error);
