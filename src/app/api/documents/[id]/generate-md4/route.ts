...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const formatDir = path.join(process.cwd(), "format", "md-4");
    ...
    for (const file of docxFiles) {
      try {
        ...
        console.error(`[generate-md4] Error processing template ${file}:`, err);
      }
    }
    ...
    return new NextResponse(zipBuffer as unknown as BodyInit, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="MD-4_Documents.zip"`,
      },
    });
  } catch (error) {
    ...
    console.error(`[generate-md4] Error:`, error);
