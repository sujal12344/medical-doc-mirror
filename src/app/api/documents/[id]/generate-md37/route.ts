...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const templatePath = path.join(process.cwd(), "format", "md-37", "05_Official_Form_MD-37_Fair_Price_Refusal_Receipt_Conditional_Template.docx");
    ...
    return new NextResponse(buf as unknown as BodyInit, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="MD-37_Fair_Price_Refusal_Receipt.docx"`,
      },
    });
  } catch (error) {
    ...
    console.error("[generate-md37] Error:", error);
