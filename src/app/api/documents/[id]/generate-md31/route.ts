...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const templatePath = path.join(process.cwd(), "format", "md-31", "01_Official_Form_MD-31_CMDTL_Test_or_Evaluation_Certificate_Template.docx");
    ...
    return new NextResponse(buf as unknown as BodyInit, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="MD-31_CMDTL_Test_Certificate.docx"`,
      },
    });
  } catch (error) {
    ...
    console.error("[generate-md31] Error:", error);
