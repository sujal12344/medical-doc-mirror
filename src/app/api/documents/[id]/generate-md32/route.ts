...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const templatePath = path.join(process.cwd(), "format", "md-32", "01_Official_Form_MD-32_Test_or_Evaluation_Report_Template.docx");
    ...
    return new NextResponse(buf as unknown as BodyInit, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="MD-32_Test_Report.docx"`,
      },
    });
  } catch (error) {
    ...
    console.error("[generate-md32] Error:", error);
