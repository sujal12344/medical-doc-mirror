...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const templatePath = path.join(process.cwd(), "format", "md-33", "01_Official_Form_MD-33_Purchaser_Test_Application_Template.docx");
    ...
    return new NextResponse(buf as unknown as BodyInit, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="MD-33_Purchaser_Test_Application.docx"`,
      },
    });
  } catch (error) {
    ...
    console.error("[generate-md33] Error:", error);
