...
export default function MD18Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleGenerate(docType: "form" | "recommendation" | "certificate") {
    if (!docId) return;
    setStatusMsg(`Generating MD-18 ${docType}...`);
    setStatusType("info");
    
    try {
      const res = await fetch(`/api/documents/${docId}/generate-md18`, {
        method: "POST",
      ...
      a.href = url;
      a.download = `${product?.name || 'product'}_MD-18_${docType}.docx`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      
      setStatusMsg(`MD-18 ${docType} generated successfully!`);
    ...
    return (
      <div className="p-8 max-w-4xl mx-auto flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted">Loading MD-18 details...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted mb-2">
            ...
            <span className="text-foreground font-medium">MD-18 Form</span>
          </div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            ...
            MD-18 Form Generation
          </h1>
          <p className="text-sm text-muted mt-1">
            Application for Licence to Manufacture for Sale. Download the required templates below.
          </p>
        </div>
      </div>
          ...
          <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-4 bg-surface2 border border-border rounded-xl">
                <div>
                  <h3 className="text-sm font-semibold text-foreground">MD-18 Form</h3>
                  <p className="text-xs text-muted mt-0.5">Application for Licence to Manufacture for Sale</p>
                </div>
                <button
              ...
              <div className="flex items-center justify-between p-4 bg-surface2 border border-border rounded-xl">
                <div>
                  <h3 className="text-sm font-semibold text-foreground">Medical Officer Recommendation</h3>
                  <p className="text-xs text-muted mt-0.5">Recommendation letter template</p>
                </div>
                <button
              ...
              <div className="flex items-center justify-between p-4 bg-surface2 border border-border rounded-xl">
                <div>
                  <h3 className="text-sm font-semibold text-foreground">Medical Superintendent Certificate</h3>
                  <p className="text-xs text-muted mt-0.5">Certificate template for Superintendent / Head</p>
                </div>
                <button
