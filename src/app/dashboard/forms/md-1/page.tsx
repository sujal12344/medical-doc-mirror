...
export default function MD1Page() {
  ...
  async function handleGenerate() {
    ...
    setGenerating(true);
    setStatusMsg("Extracting data and generating MD-1 documents — this may take a moment...");
    setStatusType("info");
    
    try {
      const res = await fetch(`/api/documents/${docId}/generate-md1`, {
        method: "POST",
      });
      ...
      setStatusMsg("MD-1 documents generated successfully! Click Download ZIP to save.");
    ...
    return (
      <div className="p-8 max-w-4xl mx-auto flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          <p className="text-sm text-muted">No document ID found. Go back and generate again.</p>
          <button onClick={() => router.push("/dashboard/forms")} className="mt-4 text-sm text-[var(--accent)] hover:underline">&larr; Back to Forms</button>
        </div>
      </div>
    );
  }
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted mb-2">
            ...
            <span className="text-foreground font-medium">MD-1 Application</span>
          </div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)]">
              <FileText className="w-4 h-4" />
            </div>
            MD-1 Documents Generation
          </h1>
          <p className="text-sm text-muted mt-1">
            Application for Registration as Notified Body
          </p>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm">
        ...
        <p className="text-sm text-muted mb-6">
          This will generate a complete set of MD-1 application templates bundled into a single ZIP archive.
        </p>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              "01_MD-1_Covering_Letter.docx",
              "02_MD-1_Constitution_Details.docx",
              "03_MD-1_Organisation_Audit_Business_Profile.docx",
              "04_MD-1_SOP_Master_List.docx",
              "05_MD-1_Technical_Personnel_and_Outside_Experts_List.docx",
              "06_MD-1_Independence_and_Conflict_of_Interest_Undertaking.docx",
              "07_Official_Form_MD-1.docx",
              "08_Quality_Manual_of_the_Organisation.docx"
            ].map((filename, i) => (
