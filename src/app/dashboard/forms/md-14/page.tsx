...
export default function MD14Page() {
  ...
  async function handleGenerate() {
    ...
    setStatusMsg(`Generating MD-14 documents...`);
    setStatusType("info");
    setLoading(true);
    
    try {
      const res = await fetch(`/api/documents/${docId}/generate-md14`, { method: "POST", headers: { "Content-Type": "application/json" } });
      if (!res.ok) {
      ...
      setDownloadUrl(url);
      setStatusMsg(`MD-14 documents generated successfully!`);
  ...
  function handleDownload() {
    ...
    a.href = downloadUrl;
    a.download = "MD-14_Documents.zip";
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted mb-2">
            ...
            <span className="text-foreground font-medium">MD-14 Form</span>
          </div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)]">
              <FileText className="w-4 h-4" />
            </div>
            MD-14 Form Generation
          </h1>
          <p className="text-sm text-muted mt-1">
            Application for import registration/license of medical device. Generate and download the required templates in a ZIP archive.
          </p>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm">
        ...
        <p className="text-sm text-muted mb-6">
          This will generate the MD-14 templates.
        </p>
