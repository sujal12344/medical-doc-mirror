...
export default function MD41Page() {
  ...
  async function handleGenerate() {
    ...
    setStatusMsg(`Generating MD-41 documents archive...`);
    setStatusType("info");
    
    try {
      const res = await fetch(`/api/documents/${docId}/generate-md41`, { method: "POST", headers: { "Content-Type": "application/json" } });
      if (!res.ok) {
      ...
      setDownloadUrl(url);
      setStatusMsg(`MD-41 documents generated successfully! Click Download ZIP to save.`);
  ...
  function handleDownload() {
    ...
    a.href = downloadUrl;
    a.download = "MD-41_Documents.zip";
    ...
    return (
      <div className="p-8 max-w-4xl mx-auto flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted">Loading MD-41 details...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted mb-2">
            ...
            <span className="text-foreground font-medium">MD-41 Form</span>
          </div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)]">
              <FileText className="w-4 h-4" />
            </div>
            MD-41 Form Generation
          </h1>
          <p className="text-sm text-muted mt-1">
            Grant of registration to medical device distributor. Download the required templates below.
          </p>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm">
        ...
        <p className="text-sm text-muted mb-6">
          This will generate a complete set of MD-41 distributor registration application templates bundled into a single ZIP archive.
        </p>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              "01_Official_Form_MD-41_Template.docx",
              "02_MD-41_Good_Distribution_Compliance_Self-Certificate_Template.docx",
              "03_MD-41_Other_Activities_at_Premises_Statement_Template.docx",
              "04_MD-41_Storage_Requirements_Compliance_Undertaking_Template.docx"
            ].map((filename, i) => (
