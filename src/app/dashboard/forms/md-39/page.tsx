...
export default function MD39Page() {
  ...
  async function handleGenerate() {
    ...
    setStatusMsg(`Generating MD-39 documents archive...`);
    setStatusType("info");
    
    try {
      const res = await fetch(`/api/documents/${docId}/generate-md39`, { method: "POST", headers: { "Content-Type": "application/json" } });
      if (!res.ok) {
      ...
      setDownloadUrl(url);
      setStatusMsg(`MD-39 documents generated successfully! Click Download ZIP to save.`);
  ...
  function handleDownload() {
    ...
    a.href = downloadUrl;
    a.download = "MD-39_Documents.zip";
    ...
    return (
      <div className="p-8 max-w-4xl mx-auto flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted">Loading MD-39 details...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted mb-2">
            ...
            <span className="text-foreground font-medium">MD-39 Form</span>
          </div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)]">
              <FileText className="w-4 h-4" />
            </div>
            MD-39 Form Generation
          </h1>
          <p className="text-sm text-muted mt-1">
            Grant of registration to common test or evaluation laboratory. Download the required templates below.
          </p>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm">
        ...
        <p className="text-sm text-muted mb-6">
          This will generate a complete set of MD-39 laboratory registration application templates bundled into a single ZIP archive.
        </p>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {[
              "01_MD-39_Covering_Letter_Template.docx",
              "02_Official_Form_MD-39_Template.docx",
              "03_MD-39_Organisation_Chart_and_Key_Personnel_Template.docx",
              "04_MD-39_Technical_Staff_and_Person_In-Charge_List_Template.docx",
              "05_MD-39_Equipment_Apparatus_and_Instruments_List_Template.docx",
              "06_MD-39_Contract_Activities_Statement_Template.docx",
              "07_MD-39_Laboratory_QMS_Requirements_Template.docx",
              "08_MD-39_Training_Needs_and_Competence_Procedure_Template.docx",
              "09_MD-39_Standard_and_Test_Method_Master_List_Template.docx",
              "10_MD-39_SOP_Master_List_Template.docx"
            ].map((filename, i) => (
