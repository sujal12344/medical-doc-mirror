...
export default function MD3Page() {
  ...
  async function handleGenerate() {
    ...
    setStatusMsg(`Generating MD-3 documents...`);
    setStatusType("info");
    setLoading(true);
    
    try {
      const res = await fetch(`/api/documents/${docId}/generate-md3`, { method: "POST", headers: { "Content-Type": "application/json" } });
      if (!res.ok) {
      ...
      setDownloadUrl(url);
      setStatusMsg(`MD-3 documents generated successfully!`);
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted mb-2">
            ...
            <span className="text-foreground font-medium">MD-3 Form</span>
          </div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)]">
              <FileText className="w-4 h-4" />
            </div>
            MD-3 Form Generation
          </h1>
          <p className="text-sm text-muted mt-1">
            Site Master File for manufacturing premises. Generate and download the required templates in a ZIP archive.
          </p>
        </div>
      </div>
