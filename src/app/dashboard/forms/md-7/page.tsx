...
export default function MD7Page() {
  ...
  async function handleGenerate() {
    ...
    setStatusMsg(`Generating MD-7 documents...`);
    setStatusType("info");
    setLoading(true);
    
    try {
      const res = await fetch(`/api/documents/${docId}/generate-md7`, { method: "POST", headers: { "Content-Type": "application/json" } });
      if (!res.ok) {
      ...
      setDownloadUrl(url);
      setStatusMsg(`MD-7 documents generated successfully!`);
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-muted mb-2">
            ...
            <span className="text-foreground font-medium">MD-7 Form</span>
          </div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)]">
              <FileText className="w-4 h-4" />
            </div>
            MD-7 Form Generation
          </h1>
          <p className="text-sm text-muted mt-1">
            Application for Grant of Licence to Manufacture for Sale or for Distribution of Class C or Class D. Generate and download the required templates in a ZIP archive.
          </p>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm">
        ...
        <p className="text-sm text-muted mb-6">
          This will generate the MD-7 templates.
        </p>
